...
- **PWA** — 홈 화면 설치 + 수제 서비스 워커(`public/sw.js`) 오프라인 캐싱 + 복습 알림(앱 열 때 하루 1회 로컬 + 옵트인 Web Push: **복습 시점(FSRS due)에 발송**, 밤 10시~아침 8시 보류, 서버는 Workmate `api/loopla/push`, 저장은 endpoint·키·로케일·다음 복습 시각·대기 장수뿐)
- **한/영 UI 토글** — 글로벌 확장 대비
